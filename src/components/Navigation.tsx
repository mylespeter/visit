  ...
  const getPageConfig = (path: string) => {
    switch (path) {
      ...
      case '/admintest':
        return {
          title: 'Admin Test',
          description: 'Page de test pour les administrateurs',
        }
      case '/profile':
        return {
          title: 'Mon profil',
...
// Dans votre composant Navigation, modifiez handleLogout :
...
const handleLogout = async () => {
  ...
  const isPastor = user.role?.nom?.toLowerCase() === 'pasteur'
  const isSecretaire = user.role?.nom?.toLowerCase() === 'secretaire'

  // Navigation links - construction conditionnelle
  const getNavLinks = () => {
    ...
    if (isPastor) {
      baseLinks.push({ href: '/pasteur/visites', label: 'Visites', icon: CalendarCheck })
    }else {
      baseLinks.push({ href: '/visites', label: 'Visites', icon: CalendarCheck })
    }

    if (isPastor) {
      baseLinks.push({ href: '/historique', label: 'Historique', icon: History })
      baseLinks.push({ href: '/rapports', label: 'Rapports', icon: ChartNoAxesCombined })
    }else {
       baseLinks.push({ href: '/historique', label: 'Historique', icon: History })
      baseLinks.push({ href: '/rapports', label: 'Rapports', icon: ChartNoAxesCombined })
    }
    ...
    if (isPastor) {
      baseLinks.push({ href: '/pasteur/tableau', label: 'Tableau', icon: Table })
    }else {
      baseLinks.push({ href: '/tableau', label: 'Tableau', icon: Table })
    }

    if (isAdmin) {
      baseLinks.push({ href: '/admin', label: 'Administration', icon: Shield })
    } else if (isSecretaire) {
      baseLinks.push({ href: '/administration', label: 'Administration', icon: Shield })
    }
    return baseLinks
  }
